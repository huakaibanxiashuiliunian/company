...
from flaskProject import cookie
...
def extract(html):
    ...
    dataList = []
    for item in data:
        data = item.get_text().replace("\n", "").replace("复制", "").replace("：", " ").replace(">", "").replace(" 至 ", "至").replace("http://","").replace("https://","").split()
        for i in range(0, len(data)):
            # print(data[i])
...
def getresource(urllist, keyList, key):
    if key in keyList:
        newkey = key
    else:
        newkey = keyList[0]
    print(newkey)
    if newkey in urllist:  # 判断 如果关键字在查出的urllist字典中 重新指定url  如果不在字典中 查询列表第一个详情网址
        url = urllist.get(newkey)
        print("-------------------------查询%s-------------------------" % newkey)
        print("-------------------------地址%s-------------------------" % url)
    print(url)
    html = askURL(url)  # 获取网页信息
    ...
    resource['单位名称'] = newkey
    return resource

def getofficialurl(key):
    ...
    print(urllist)
    print(keyList)
    resource = getresource(urllist, keyList, key)
    print(resource)
    print('官网：' + resource['官网'])
    officialurl = resource['官网']
    return officialurl
